...
export const HandSkill = {
     ...
     */
    sha({ user, target, discardPile }: SkillCommonParam): void {
        ...
        // 目标玩家扣血
        target && (target.general.currentHealth -= 1 + user.drinkingCount)
     ...
     * 闪
     */
    shan({ user, discardPile }: SkillCommonParam): void {
        findChoosedHandAndDiscarded({ user, discardPile })
    },
     ...
     * @param discardPile 弃牌堆
     */
    tao({ user, discardPile }: SkillCommonParam): void {
        findChoosedHandAndDiscarded({ user, discardPile })
     ...
     *
     */
    jiu({ user, discardPile }: SkillCommonParam): void {
        findChoosedHandAndDiscarded({ user, discardPile })
